...
int outputGeo(MeshRegions &combinedReg, MeshRegions &nearWallRegion,
              ...
              vector<double> pC);
int meshingInFoil_v2(MeshRegions &nearWallRegion, MeshRegions &inFoilRegion,
                     vector<vector<double>> &breakpts);
...
int meshingWake(MeshRegions &combinedReg) {
  ...
  pwakeLU.push_back(pts[14][0]);
  pwakeLU.push_back(pts[14][1]);
  transform(pwakeLU, AoA);
  pts[16][1] = farWakeDown;
  pts[19][1] = farWakeUp;
  double gapofFarWakeL =
      1.8 * (pts[19][1] - pts[16][1]) / nFarWakey + 0. * pts[14][1] / nLayers;
...
int meshingInFoil_v2(MeshRegions &nearWallRegion, MeshRegions &inFoilRegion,
  ...
  for (int i = 1; i < nWallPts - 1; ++i) {
    ...
    double l1, l2;
    t1[0] = nearWallRegion.m_pts[boundary[wallID][i]][0] -
            nearWallRegion.m_pts[boundary[wallID][i - 1]][0];
    t1[1] = nearWallRegion.m_pts[boundary[wallID][i]][1] -
            nearWallRegion.m_pts[boundary[wallID][i - 1]][1];
    t2[0] = nearWallRegion.m_pts[boundary[wallID][i + 1]][0] -
            nearWallRegion.m_pts[boundary[wallID][i]][0];
    t2[1] = nearWallRegion.m_pts[boundary[wallID][i + 1]][1] -
            nearWallRegion.m_pts[boundary[wallID][i]][1];
  ...
  for (int i = nWallPts / 2; i < nWallPts / 2 + nWallPts; ++i) {
    if (nearWallRegion.m_pts[boundary[wallID][i % nWallPts]][0] >
        1. - 1.5 * hTrailingEdge) {
  ...
  for (int i = 0; i < nWallPts - 1; ++i) {
    ...
    for (int k = 0; k <= nl; ++k) {
      vector<double> p0(2);
      vector<double> p1(2);
      p0[0] = nearWallRegion.m_pts[boundary[wallID][i]][0] +
              wallnorm[i][0] * sig *
                  radiusEdgeInFoil(k * 2. / nLayersInFoil - 1.)[0];
      p0[1] = nearWallRegion.m_pts[boundary[wallID][i]][1] +
              wallnorm[i][1] * sig *
                  radiusEdgeInFoil(k * 2. / nLayersInFoil - 1.)[0];
      p1[0] = nearWallRegion.m_pts[boundary[wallID][i + 1]][0] +
              wallnorm[i + 1][0] * sig *
                  radiusEdgeInFoil(k * 2. / nLayersInFoil - 1.)[0];
      p1[1] = nearWallRegion.m_pts[boundary[wallID][i + 1]][1] +
              wallnorm[i + 1][1] * sig *
  ...
  double xc = 0.5 * (nearWallRegion.m_pts[trailingedge[0]][0] +
                     nearWallRegion.m_pts[trailingedge[nround - 1]][0]);
...
int outputXML(MeshRegions &combinedReg, MeshRegions &inFoilRegion) {
  ...
  combinedReg.defineBoundary((void *)edge6, Cedge6.m_N, 0, curvedpts, AoA, 1,
                             (void *)roundTrailingEdge);
  combinedReg.defineBoundary((void *)edgeb2_b7, Cedgeb2_b7.m_N, 0, curvedpts,
                             AoA, 1, (void *)roundTrailingEdge);
  // inlet
