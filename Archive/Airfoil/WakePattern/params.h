...
void DefineBLParams(std::map<std::string, double> &p,
  ...
  int nUp0 =
      std::min(30, std::max(int(0.5 * (upperBL0 + 0.5 * Thickness) * M_PI / maxLayerh + 0.5), 10));
  q["nUp0"] = nUp0;
  int nUp1 = (upperx1) / maxLayerh + 2;
  q["nUp1"] = nUp1;
  int nUp2 = (upperx2 - upperx1) / maxLayerh + 3;
  q["nUp2"] = nUp2;
  int nUp3 = (upperx3 - upperx2) / maxLayerh + 3;
  q["nUp3"] = nUp3;
  int nUp4 = (ChordLen - upperx3) / maxLayerh + 6;
  q["nUp4"] = nUp4;
  ...
  q["nUp5"] = nUp5;
  int nLow0 =
      std::min(30, std::max(int(0.5 * (lowerBL0 + 0.5 * Thickness) * M_PI / maxLayerh + 0.5), 10));
  q["nLow0"] = nLow0;
  int nLow1 = (lowerx1) / maxLayerh + 2;
  q["nLow1"] = nLow1;
  int nLow2 = (lowerx2 - lowerx1) / maxLayerh + 3;
  q["nLow2"] = nLow2;
  int nLow3 = (lowerx3 - lowerx2) / maxLayerh + 3;
  q["nLow3"] = nLow3;
  int nLow4 = (ChordLen - lowerx3) / maxLayerh + 6;
  q["nLow4"] = nLow4;
