...
FOILTYPE foil(chamber, chamberp, Thickness);
int nTrailingEdge =
    std::max(2, (int)ceil((foil.up(chordLen)[1] - foil.down(chordLen)[1]) /
                          hFirstLayer));
double pts[NUMPTS][2];
...
int InitPts() {
  ...
  pts[3][0] = foil.down(xmidLow1)[0];
  pts[3][1] = foil.down(xmidLow1)[1];

  pts[4][0] = foil.down(xmidLow2)[0];
  pts[4][1] = foil.down(xmidLow2)[1];

  pts[5][0] = foil.up(xmidUp2)[0];
  pts[5][1] = foil.up(xmidUp2)[1];

  pts[6][0] = foil.up(xmidUp1)[0];
  pts[6][1] = foil.up(xmidUp1)[1];
  ...
  virtualpts[3][1] = foil.up(xmidLow1)[1];

  virtualpts[4][0] = xmidLow2;
  virtualpts[4][1] = foil.up(xmidLow2)[1];

  virtualpts[5][0] = xmidUp2;
  virtualpts[5][1] = foil.up(xmidUp2)[1];

  virtualpts[6][0] = xmidUp1;
  virtualpts[6][1] = foil.up(xmidUp1)[1];
  return 0;
}

std::vector<double> roundTrailingEdge(std::vector<double> p) {
  // return p;
  return foil.roundTrailingEdge(p);
}
...
LineEdge Cedge1(pts[1], pts[2], nWake, BOUNDARYLAYER1, 0., 0., 0,
                hTrailingEdge + hFirstLayer, 2., 2);
LineEdge Cedge7(pts[7], pts[0], nWake, BOUNDARYLAYER0,
...
std::vector<double> edge8(double s) { return Cedge8.Evaluate(s); }
...
LineEdge Cedge14(pts[12], pts[13], nWake, BOUNDARYLAYER0,
                 hTrailingEdge + hFirstLayer, 2., 2, 0., 0., 0);
LineEdge Cedge15(pts[13], pts[1], nLayers1 - 1, BOUNDARYLAYER1, 0., 0., 0,
                 hFirstLayer *progress, progress, (nLayers1 - 1) * 2 / 3);
LineEdge Cedge16(pts[0], pts[14], nLayers7 - 1, BOUNDARYLAYER0,
                 hFirstLayer *progress, progress, (nLayers7 - 1) * 2 / 3, 0.,
                 0., 0);
LineEdge Cedge17(pts[14], pts[15], nWake, BOUNDARYLAYER1, 0., 0., 0,
                 hTrailingEdge + hFirstLayer, 2., 2);
std::vector<double> edge14(double s) { return Cedge14.Evaluate(s); }
std::vector<double> edge15(double s) { return Cedge15.Evaluate(s); }
std::vector<double> edge16(double s) { return Cedge16.Evaluate(s); }
std::vector<double> edge17(double s) { return Cedge17.Evaluate(s); }
// imaged edge
...
LineEdge Cedgeb2_b7(virtualpts[7], virtualpts[2], nTrailingEdge, UNIFORM, 0.,
                    0.);
